...
#define TYPE_COMMAND_UPDATE_FW "update_fw"
...
bool json_parser_message(const char *message, uint8_t source)
{
	...
	if (command)
	{
		...
		else if ((strcmp(value_type_cmd, TYPE_COMMAND_UPDATE_FW) == 0))
		{
			if (device_data.ota_status == E_OTA_NONE)
			{
				APP_LOGD("update_fw start");
				device_data.ota_status = E_OTA_START;
			}
			status = true;
		}
	}
...
static bool json_parser_command_contrl(const char *message)
{
	...
	if (Payload)
	{
		...
		if (type_mode > E_MODE_PIEZO)
		{
			type_mode = E_MODE_PIEZO;
		}
